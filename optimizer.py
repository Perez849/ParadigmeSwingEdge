"""
╔══════════════════════════════════════════════════════════════════════╗
║  SWING EDGE OPTIMIZER  v3.0                                          ║
║  Optuna (Bayesian) · Walk-forward · Caché JSON · Compatible Jupyter  ║
╚══════════════════════════════════════════════════════════════════════╝

CAMBIOS v3:
  · Optimización bayesiana con Optuna en lugar de muestreo aleatorio
  · Mismo número de evaluaciones (N_COMBOS) pero dirigidas inteligentemente
  · Optuna aprende de cada evaluación y concentra la búsqueda en zonas buenas
  · Eliminada semilla fija — cada optimización explora zonas distintas del espacio
  · Mismo objetivo: maximizar score OOS penalizando degradación IS→OOS
  · 100% compatible con engine.py y optimal_params.json existente
...
try:
    ...
    raise ImportError("Instala optuna primero: pip install optuna")
...
DATA_PERIOD = "5y"
N_COMBOS    = 4000   # evaluaciones por activo — más evaluaciones = mejor exploración bayesiana
...
# ══════════════════════════════════════════════════════════════════
# BLOQUE COMPARTIDO — COPIA EXACTA DE engine.py
# ══════════════════════════════════════════════════════════════════
...
def optimize(ticker, df, macro, n_combos):
    ...
    def objective(trial):
        p = {k: trial.suggest_categorical(k, v) for k, v in SEARCH.items()}

        # Restricciones estructurales
        if p['ema_f'] >= p['ema_s'] or p['macd_f'] >= p['macd_s']:
            return -999.0

        try:
            # Evaluar IS
            ind_is  = calc_ind(df_is, p, use_invert)
            sigs_is = get_signals(ind_is, p, mo_is, use_invert)
            if sigs_is.sum() < 3:
                return -999.0
            trs_is = run_bt(ind_is, sigs_is, p)
            m_is   = score_metrics(trs_is)
            if not m_is:
                return -999.0
            ...
            # Mismo objetivo que v2: score OOS penalizando degradación
            deg = max(0, m_is['score'] - m_oos['score'])
            return m_oos['score'] - deg * 0.2

        except Exception:
            return -999.0

    # TPE sampler: 200 trials aleatorios para explorar, resto bayesianos
    sampler = optuna.samplers.TPESampler(n_startup_trials=200, n_ei_candidates=100)
    study   = optuna.create_study(direction='maximize', sampler=sampler)
    study.optimize(objective, n_trials=n_combos, show_progress_bar=False)

    best = study.best_trial
    if best.value <= -999.0:
        return None

    bp = {k: best.params[k] for k in SEARCH.keys()}

    # Recalcular métricas finales limpias
    try:
        ind_is   = calc_ind(df_is,  bp, use_invert)
        sigs_is  = get_signals(ind_is,  bp, mo_is,  use_invert)
        m_is     = score_metrics(run_bt(ind_is,  sigs_is,  bp))

        ind_oos  = calc_ind(df_oos, bp, use_invert)
        sigs_oos = get_signals(ind_oos, bp, mo_oos, use_invert)
        m_oos    = score_metrics(run_bt(ind_oos, sigs_oos, bp))
    except Exception:
        return None
    ...
    return dict(
        ...
        score_is    = round(m_is['score'],  3),
        score_oos   = round(m_oos['score'], 3),
...
def main():
    ...
    tickers = [TICKER_SOLO] if TICKER_SOLO else list(UNIVERSE.keys())
    ...
    for ticker in tickers:
        ...
        if ticker in cache and not FORCE_REOPT:
            m   = cache[ticker].get('metrics_oos', {})
            ver = cache[ticker].get('optimizer', 'random_v2')
            print(f"  ⏭  Caché [{ver}] ({cache[ticker].get('optimized_at','')[:10]})  "
        ...
        t0  = time.time()
        res = optimize(ticker, df, macro, N_COMBOS_RUN)
        dt  = time.time() - t0

        if res:
            m = res['metrics_oos']
            print(f"✓ {dt:.0f}s  "
                  f"Sharpe={m['sharpe']:.2f}  PF={m['pf']:.2f}  "
